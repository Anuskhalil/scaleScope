    ...
    <>
      ...
      <div className="min-h-screen page-bg pt-20 pb-16 dm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* ── SEARCH ──────────────────────────────────────────────── */}
          <div className="search-glow rounded-2xl transition-all mb-6 f1">
            <div className="flex items-center bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              ...
              <button className="g-ind m-2 text-white px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 hover:opacity-90 transition-all">
                  ...
                  <div className="flex gap-2 overflow-x-auto no-scroll pb-2 mb-5">
                    {OPP_TYPES.map(type => (
                      <button key={type} onClick={()=>setOppTypeFilter(type)}
                        className={`text-xs font-semibold px-3.5 py-2 rounded-xl whitespace-nowrap capitalize transition-all ${
                          oppTypeFilter===type
                            ?'bg-indigo-600 text-white shadow-sm'
                            :'bg-white border border-slate-200 text-slate-600 hover:border-indigo-200 hover:text-indigo-600'
                        }`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={onConnect} disabled={connecting==='sent'}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold transition-all ${
            connecting==='sent'?'bg-emerald-50 text-emerald-600 border-2 border-emerald-200':
          ...
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
              <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${
                isMentor?'bg-indigo-50 text-indigo-600 border border-indigo-100':'bg-violet-50 text-violet-600 border border-violet-100'
              }`}>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {person.skills.slice(0,4).map((sk,i) => (
                <span key={i} className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${
                  isMentor?'bg-indigo-50 text-indigo-600':'bg-violet-50 text-violet-600'
                }`}>{sk}</span>
              ))}
            </div>
          )}
